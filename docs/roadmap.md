...
2026-09-17, and the first UK entries were added on 2026-09-23. Anything in
...
- [x] Seed the directory with its first entries, each verified against a
  live homepage. (34 entries across 14 domains, 2026-09-23)
- [ ] Fill the empty domains listed in [docs/known-gaps.md](known-gaps.md):
  ...
  citizen science, food rescue and food security tech, and makerspaces and
  hackerspaces.
...
- [ ] Backfill `github` and `linkedin_org` where the organisation has a real
  public one. All 34 seeded entries have both fields empty.
- [ ] Backfill `founding_year` where it can be read off an about page or
  another official source. 33 of 34 entries currently have it null.
- [ ] Backfill `takes_contributors` where the organisation publishes a
  volunteering or open-source contribution page. All 34 entries are null.
- [ ] Backfill `careers_url` where the organisation has a careers, jobs, or
  volunteering page. All 34 entries are empty.
- [ ] Cross-link entries via `related_to` where real, verifiable connections
  ...
  links today.
- [ ] Add UK community channels and event series as entries once each is
  confirmed active. The Tech for Good Organisers Network is a candidate. See
...
- [x] Normalise region values against a fixed list in the schema `enum` so
  filtering and mapping stay reliable. (13 UK regions plus `national`,
  2026-09-23)
...
- [ ] Research UK tech-for-good meetups, Slack or Discord communities, and
...
- Scaffolded the repo from nz-tech-for-good: removed the NZ data, replaced
  the NZ region and domain taxonomy with UK equivalents, and repointed every
  hardcoded repo and site URL. (2026-09-23)
- First 34 UK entries, each checked against a live homepage on the day it was
...
- Rewrote the README, `docs/how-entries-are-chosen.md`,
  ...
  issue form for the UK. (2026-09-23)
