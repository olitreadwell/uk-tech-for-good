...
### Added: 2026-09-23 (UK scaffold)

- Seeded the directory with 34 UK entries, each verified against its own
  homepage on the day it was added. Regenerated `GUIDE.md` and the JSON and
  CSV exports from them.
- Rewrote the README for the UK, including a status section that says plainly
  that the directory has just been seeded.
- Rewrote `docs/how-entries-are-chosen.md`, `docs/known-gaps.md`,
  `docs/research-get-involved.md`, and `docs/roadmap.md` for the UK.
- Repointed every hardcoded repository and site URL at
  `olitreadwell/uk-tech-for-good` and `uk-tech-for-good.vercel.app`.

### Changed: 2026-09-23 (UK scaffold)

- Replaced the NZ region list in `schema/entry.schema.json` with 13 UK
...
- Removed the three NZ-specific domains (Māori data sovereignty, iwi and
  ...
  add-entry issue form. Every remaining domain has an explainer.
- Renamed the root package from `@olitreadwell/nz-tech-for-good` to
  `@olitreadwell/uk-tech-for-good`.
- Cleared `data/archives.json`, which held Wayback snapshots of NZ entries.

### Removed: 2026-09-23 (UK scaffold)

- Removed the 132 NZ directory entries, the NZ `GUIDE.md`, and the NZ
  exports, which the scaffold commit had left in place.
